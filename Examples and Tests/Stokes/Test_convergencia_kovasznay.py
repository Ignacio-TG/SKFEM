...
import pandas as pd
from build_Kovasznay import build_and_solve_stokes, u_exact_x, u_exact_y, p_exact_xy, f_exact_x, f_exact_y
...
def err_H1_semi_u(w):
    ...
    dux_dx_e = -lamb * np.exp(lamb*x) * np.cos(2*np.pi*y)
    dux_dy_e =  2*np.pi * np.exp(lamb*x) * np.sin(2*np.pi*y)

    duy_dx_e = (lamb**2/(2*np.pi)) * np.exp(lamb*x) * np.sin(2*np.pi*y)
    duy_dy_e =  lamb * np.exp(lamb*x) * np.cos(2*np.pi*y)
...
df.to_csv('convergencia_stokes_kovasznay.csv', index=False)